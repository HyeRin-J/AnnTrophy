...
struct Vector2 : double2
{
	...
	Vector2(double2& V);
	Vector2(D2D1_POINT_2F p1, D2D1_POINT_2F p2);
	//Vector2(Vector2& F);
	...
	Vector2 Normalize();
	static void Normalize(Vector2& result);
	...
	static float Distance(float _myX, float _myY, float _targetX, float _targetY);
