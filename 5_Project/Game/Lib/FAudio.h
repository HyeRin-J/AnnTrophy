...
/// </summary>
class FAudio : public SingletonManager<FAudio>
{
	//FMOD::MAPPING
public:
...
public:
	...
	*/
	...
	bool FCreateSound(std::wstring _name,bool _loop, FMOD::Sound** _sonund, std::wstring _folder = _T(""));
	//사운드를 추가해주는 함수이다. //함수이름만 입력하면된다.
	bool FAddSound(std::wstring, bool _loop, std::wstring _folder = _T(""));
	bool FAddBackground(std::wstring, bool _loop, FMOD::Channel** , std::wstring _folder = _T(""));
	//현재 이름의 사운드를 재생
	...
	bool FUpdate();
	//파일 찾고 그 기능을 실행할 함수
	bool FFindChannel(std::wstring& _path , SOUND_PLAY _Set);
	...
	//--------------------------------------
	//기타등등 이펙트 추가시 작업할 공간
	bool FIsPlaying(std::wstring _path);
